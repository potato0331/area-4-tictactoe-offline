class area4 {
    ...
    getPosition(offsetX,offsetY){
        return {'inputX':Math.floor((offsetX-this.sideMargin)/this.blockinterval),'inputY':Math.floor((offsetY-this.sideMargin)/this.blockinterval)}
    }

    getBoardPosition(inputX,inputY){
        return {'boardX':inputX*this.blockinterval+this.blockinterval/2+this.sideMargin, 'boardY':inputY*this.blockinterval+this.blockinterval/2+this.sideMargin}
    }
    ...
    checkAvailableMove(MoveInfo){
        ...
        if(this.mainBoard.length == 0){ //첫 수일경우
            for (let i = 0; i < 81;i++){
                if (!(i % 9 > 1 && i % 9 < 7 && Math.floor(i / 9) > 1 && Math.floor(i / 9) < 7)){
            ...
            let AvailableArea=[NWsquare, NWsquare + 1, NWsquare + 2, NWsquare + 9 ,NWsquare + 10 ,NWsquare + 11, NWsquare + 18, NWsquare + 19, NWsquare + 20]
            // 착수 가능한 영역 안 칸들의 인덱스 리스트
            for (let i of AvailableArea){
                let occupied = this.mainBoard.find(point => point.x == i % 9 && point.y == Math.floor(i / 9))
    ...
    drawboard(ctx){
        ...
        for (let i = 0; i < 9 ; i++){
            for (let j = 0; j < 9; j++){
                //정사각형그리기
                ctx.strokeRect(i*this.blockinterval+this.sideMargin,j*this.blockinterval+this.sideMargin,this.blockinterval,this.blockinterval);
            }
        }
        ...
        for (let i = 0; i < 3 ; i++){
            for (let j = 0; j < 3; j++){
                //큰정사각형그리기
                ctx.strokeRect(3*i*this.blockinterval+this.sideMargin,3*j*this.blockinterval+this.sideMargin,3*this.blockinterval,3*this.blockinterval);
            }
        }

        ctx.lineWidth=2;
        ctx.strokeStyle='red'
        ctx.strokeRect(2*this.blockinterval+this.sideMargin,2*this.blockinterval+this.sideMargin,5*this.blockinterval,5*this.blockinterval);
        //첫수 금지선 그리기   
        /** 여기까지 보드판 영역**/
        ...
        this.drawAvailableMove(ctx, this.checkAvailableMove(this.mainBoard[this.mainBoard.length-1]));
    }
}
